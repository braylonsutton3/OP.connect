...
APP_VERSION = "2026.09.11-full-confluence-news-realtime"
...
app = FastAPI(title="OP.exe Topstep Live", version=APP_VERSION)
...
def secondary_evidence(df: pd.DataFrame, tf: str):
    ...
    # VWAP is meaningful mainly intraday.
    out["VWAP"] = (
    ...
    # Opening range breakout: 09:00-09:30 America/New_York for intraday only.
    orb = 0
    if tf not in ("1d", "1w") and isinstance(df.index, pd.DatetimeIndex):
        try:
            eastern = df.tz_convert("America/New_York") if df.index.tz else df.tz_localize("UTC").tz_convert("America/New_York")
            today = eastern.index[-1].date()
            mask = (
                (eastern.index.date == today)
                & (eastern.index.time >= pd.Timestamp("09:00").time())
                & (eastern.index.time < pd.Timestamp("09:30").time())
            )
            orb_df = eastern.loc[mask]
            if len(orb_df):
                oh = float(orb_df["High"].max())
                ol = float(orb_df["Low"].min())
                if row["Close"] > oh:
                    orb = 1
                elif row["Close"] < ol:
                    orb = -1
    ...
    # Prior day/week level location.
    prior_levels = 0
    try:
        if isinstance(df.index, pd.DatetimeIndex):
            z = df.copy()
            local = z.tz_convert("America/New_York") if z.index.tz else z.tz_localize("UTC").tz_convert("America/New_York")
            ...
            if len(daily) >= 2:
                ...
                bull += int(row["Close"] > pdh); bear += int(row["Close"] < pdl)
            if len(weekly) >= 2:
                pwh = float(weekly["High"].iloc[-2]); pwl = float(weekly["Low"].iloc[-2])
                bull += int(row["Close"] > pwh); bear += int(row["Close"] < pwl)
            prior_levels = 1 if bull > bear else -1 if bear > bull else 0
    except Exception:
        prior_levels = 0
    out["PRIOR_LEVELS"] = prior_levels

    # Volatility regime: direction from close vs 20-bar mean only when ATR is elevated.
    vol_dir = 0
...
def trade_plan(df: pd.DataFrame, signal: str, primary: dict) -> Optional[dict]:
    ...
    highs, lows = confirmed_swings(df)
    recent_highs = [p for _, p in highs[-10:]]
    recent_lows = [p for _, p in lows[-10:]]
    gaps = [g for g in detect_fvgs(df, 180) if not g["filled"]]
    ...
    if signal == "LONG":
        for p in recent_highs:
            if p > entry + 0.35 * atrv:
                targets.append((p, "Confirmed swing high"))

        for g in gaps:
            if g["dir"] < 0 and g["low"] > entry:
                targets.append((g["low"], "Opposing bearish FVG"))

        for p in recent_lows:
            if p < entry and entry - p >= 0.35 * atrv:
                stops.append((p - 0.10 * atrv, "Below confirmed swing low"))

        if primary.get("_sweep_extreme") is not None:
            p = primary["_sweep_extreme"]
            if p < entry:
                stops.append((p - 0.10 * atrv, "Below liquidity sweep extreme"))
        ...
        for p in recent_lows:
            if p < entry - 0.35 * atrv:
                targets.append((p, "Confirmed swing low"))

        for g in gaps:
            if g["dir"] > 0 and g["high"] < entry:
                targets.append((g["high"], "Opposing bullish FVG"))

        for p in recent_highs:
            if p > entry and p - entry >= 0.35 * atrv:
                stops.append((p + 0.10 * atrv, "Above confirmed swing high"))

        if primary.get("_sweep_extreme") is not None:
            p = primary["_sweep_extreme"]
            if p > entry:
                stops.append((p + 0.10 * atrv, "Above liquidity sweep extreme"))
...
<title>OP.exe Realtime Full Confluence</title>
<style>
...
<body>
<div class="wrap">
<h1>OP.exe — Realtime Full Confluence</h1>
<div class="muted">Realtime TopstepX/ProjectX • PRIMARY: BOS + Liquidity Sweeps + SMA20/50 + FVG + Market News • Secondary confluence stack retained</div>
...
async function run(){
 ...
 for(let r of data.results){
  ...
  det+=`<details>
   ...
   ${r.trade?`<p>
    Entry ${n(t.entry,4)} • TP ${n(t.take_profit,4)} • SL ${n(t.stop_loss,4)} • R:R ${n(t.rr,2)}:1<br>
    TP basis: ${t.target_basis}<br>SL basis: ${t.stop_basis}
   </p>`:""}
 ...
 document.getElementById("table").innerHTML=`<table>
  <tr><th>TF</th><th>Signal</th><th>Strength</th><th>Price</th><th>Newest source</th><th>Stream age</th><th>News</th><th># headlines</th><th>Agreement</th><th>Primary 4</th><th>Entry</th><th>TP</th><th>SL</th><th>R:R</th><th>Proj. profit</th><th>Proj. risk</th></tr>${rows}
 </table>`;
